...
make pre-commit-install  # Syncs hook configs into .pre-commit-config.yaml AND installs the git hook
```

`pre-commit-install` runs the sync step (same as `make pre-commit-hooks`) and then `pre-commit install` on the host to write `.git/hooks/pre-commit`. Without that second step, git won't actually invoke the hooks on commit.

This requires the `pre-commit` binary on the host (it's the only target that breaks the hermetic-Docker pattern, since the installed hook references the host's `pre-commit` path). Install it once with `pip install pre-commit` or `brew install pre-commit`.
...
| `pre-commit-install` | Run `pre-commit-hooks` then `pre-commit install` on the host to wire up `.git/hooks/pre-commit` |
